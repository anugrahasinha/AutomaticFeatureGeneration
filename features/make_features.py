...
print("NTI LOG : Refer log file = ./ntilog/nti_"+str(newIteratorValue)+".log")
...
def asinha_exit(func_name=None,line_num=None):
    print("$$$$$$ DEBUG ASINHA : Called exit @ func_name = %s & line_num = %s" %(str(func_name),str(line_num)))
    exit(0)

def asinha_raise_exception(func_name=None,line_num=None):
    print("$$$$$$ DEBUG ASINHA : Called raise exception @ func_name %s & line_num = %s" %(str(func_name),str(line_num)))
    raise Exception("$$$$$$ DEBUG ASINHA : Called raise exception @ func_name " + str(func_name) + " & line_num = " + str(line_num))
...
def make_one_to_one_features(db, table, caller, depth):
    ...
    for related, fk in table.get_child_tables():
        if table.is_one_to_one(related, fk):
            ntiGlb.ntiGlobalObj.log("in make_one_to_one features, for table name = %s, related table = %s is one_to_one" %(table.name,related.name))
            one_to_one_table = db.tables[fk.parent.table.name]
            ntiGlb.ntiGlobalObj.log("in make_one_to_one features, finally, for table name = %s, one_to_one table = %s" %(table.name,one_to_one_table.name))
            if one_to_one_table in [table, caller]:
                print "Cannot execute one_to_one for ", table, " " , caller, '\n'
                ntiGlb.ntiGlobalObj.log("in make_one_to_one features, one_to_one_table = %s, is the caller or current table name, current table = %s & caller = %s SKIPPING IT....." %(one_to_one_table.name,table.name,str(caller)))
                continue
            print "Executing one_to_one for ", table, " " , caller, '\n'
            ntiGlb.ntiGlobalObj.log("in make_one_to_one feature, table = %s, caller = %s & one_to_one_table = %s, now applying one_to_one functions" %(table.name,str(caller),one_to_one_table.name))
...
if __name__ == "__main__":
    ...
    print("############# INFORMATION ################")
    print("This is the old make_features file, please use ntiDSM.py for running analysis")
    proceed_answer=raw_input("Do you still wish to proceed with this file (Y/N) : ")
    if proceed_answer == "N" or proceed_answer == "n":
        exit(0)
    ...
    #config_file = ["kdd2015_config.py"]
    config_file = ["blank_config.py"]
    user_input_config = raw_input("Enter the config file (saved in config directory) (default : blank_config.py) : config/")
